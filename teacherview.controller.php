                    ...
                    if ($conflicts) {
                        if (!$data->forcewhenoverlap) {
                            ...
                            foreach ($conflicts as $aconflict) {
                                ...
                                echo '<li> ' . userdate($conflictinfo->starttime) . ' ' . usertime($conflictinfo->starttime) . ' ' . get_string('incourse', 'scheduler') .
                                    ': ' . $conflictinfo->shortname . ' - ' . $conflictinfo->fullname . "</li>\n";
                            }
                            echo '</ul><br/>';
        ...
        if ($slot = $DB->get_record('scheduler_slots', array('id' => $slotid))) {
            ...
            if ($scheduler->allownotifications && $oldstudents) {
                foreach ($oldstudents as $oldstudent) {
                    ...
                    send_email_from_template($student, $teacher, $COURSE, 'cancelledbyteacher', 'teachercancelled', $vars, 'scheduler');
                }
            }
