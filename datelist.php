...
if ($numrecords) {
    ...
    $tablecolumns = array(  'courseshort', 'schedulerid', 'starttime', 'appointmentlocation', 'CONCAT(studentlastname,studentfirstname)',
                            'department', 'notes', 'grade', 'appointmentnote');
    $tableheaders = array(  "<b>$coursestr</b>", "<b>$schedulerstr</b>", "<b>$whenstr</b>", "<b>$wherestr</b>", "<b>$whostr</b>",
                            "<b>$wherefromstr</b>", "<b>$whatstr</b>", "<b>$whatresultedstr</b>", "<b>$whathappenedstr</b>");
    ...
    foreach ($results as $id => $row) {
        ...
        $whendata = ($whenmem != "$row->starttime $row->duration") ? '<strong>'.date("d M Y G:i", $row->starttime).' '.
            get_string('for', 'scheduler')." $row->duration ".get_string('mins', 'scheduler').'</strong>' : '';
        $whenmem = "$row->starttime $row->duration";
        $whodata = ($whomem != $row->studentmail) ? "<a href=\"{$CFG->wwwroot}/mod/scheduler/view.php?what=viewstudent&a={$row->schedulerid}&amp;studentid=$row->studentid&amp;" .
            "course=$row->courseid\">".$row->studentfirstname.' '.$row->studentlastname.'</a>' : '';
